...
//std::string HashGenerator::generateSHA256HashS(const std::string& input) {
//    HCRYPTPROV hProv;
//    HCRYPTHASH hHash;
//
//    if (!CryptAcquireContext(&hProv, nullptr, nullptr, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
...
std::wstring HashGenerator::generateSHA256HashW(const std::wstring& input) {
    HCRYPTPROV hProv;
    HCRYPTHASH hHash;
    ...
    if (!CryptHashData(hHash, reinterpret_cast<const BYTE*>(input.c_str()), input.length() * sizeof(wchar_t), 0)) {
        std::cerr << "Error hashing data" << std::endl;
        CryptDestroyHash(hHash);
        CryptReleaseContext(hProv, 0);
        return L"";
